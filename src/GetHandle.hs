...
instance FromJSON RawHandleAddr where
  parseJSON = withObject "RawhandleAddr"
    $ fmap Address . (.: "address") <=< (.: "tx_out")

query :: TL.Text -> Query
query hexHandleName =
  [ ( "select"
    , Just "id,tx_out!inner(id,address),multi_asset!inner(id,policy,name)")
  , ("order", Just "id.desc")
  , ("limit", Just "1")
  , ( "multi_asset.policy"
    , Just "eq.\\xf0ff48bbb7bbe9d59a40f1ce90e9e9d0ff5002ec48f232b49ca0fb9a")
  , ( "multi_asset.name"
    , Just $ "eq.\\x" <> B.toStrict (TL.encodeUtf8 hexHandleName))]
...
handleHandler inputHandle = do
  ...
  let request = setRequestQueryString (query $ removeDollar inputHandle)
        $ setRequestSecure (https cfg) request'
